...
if (Login::isloggedin()){
	...
	if (isset($_POST['confirmNewP'])){
		...
		if (password_verify($oldpword, DB::query('SELECT pword FROM admin WHERE admin_id = :adminid', array(':alumniid' => $alumniid))[0]['pword'])){
			
			if ($newpword == $newpword2){
				
				if(strlen($newpword) >= 6 && strlen($newpword) <= 32){
					
					DB::query('UPDATE admin SET pword = :newpword WHERE admin_id = :adminid', array(':adminid' => $alumniid, ':newpword' => password_hash($newpword, PASSWORD_BCRYPT)));
