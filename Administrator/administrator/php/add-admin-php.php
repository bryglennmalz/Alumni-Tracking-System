<?php
	...
	if (isset($_POST['AdminSave'])){
		...
		$afname = convert_string('encrypt', $_POST['f-name']);
		$ami = convert_string('encrypt', $_POST['mi']);
		$alname = convert_string('encrypt', $_POST['l-name']);
		$anameext = convert_string('encrypt', $_POST['ext-name']);
		$email = convert_string('encrypt', $_POST['email']);
		...
		if (!DB::query('SELECT id FROM alumni.staff WHERE id = :adminid', array(':adminid' => $adminid))){
			...
			if(filter_var($email, FILTER_VALIDATE_EMAIL)){
				...
				if(strlen($password) >= 6 && strlen($password) <= 32){
					...
					DB::query('INSERT INTO atis.admins VALUES ('/'/, :adminid, :afname, :ami, :alname, :anameext, :email, :password, :verified)', 
					...
					$forum = DB::query("SELECT * FROM atis.admins");
					$staff_lists = "";
