<?php
	...
	//Number of events query
		$cntevent =("SELECT `events`.`event_id` FROM `events`");
	...
	//Number of events this year query
		$cntev_year =("SELECT `events`.`eventid` FROM `events` WHERE events.event_date BETWEEN :yearst AND :yeard");
	...
	//Number of events this month query
		$cntev_mo =("SELECT `events`.`eventid` FROM `events` WHERE events.event_date BETWEEN :yearst AND :yeard");
	...
	//Number of events this day
		$cntev_d =("SELECT `events`.`eventid` FROM `events` WHERE events.event_date = :yearst");
	...
	//Number of upcomming events query
		$cntev_up =("SELECT `events`.`eventid` FROM `events` WHERE events.event_date BETWEEN :yearst AND :yeard");
